    ...
    <header>
      <span className="block text-xs leading-none mb-4 uppercase text-gray-400">
        ...
        <Link href={`/blog/category/${stringToSlug(attributes.category)}`}>
          <a className="font-bold text-orange-600 hover:underline">
            {attributes.category}
          </a>
        </Link>{" "}
        &ndash;&nbsp;
      ...
      <p className="text-xl md:text-3xl mt-4 md:mt-6 max-w-4xl">
        {attributes.description}
      </p>
      {attributes.leadImage?.image && (
        <figure className="mt-8">
          <img
            src={attributes.leadImage.image}
            alt={attributes.leadImage.alt}
          />
          <figcaption className="mt-2 text-gray-500 text-sm">
            {attributes.leadImage.caption}
          </figcaption>
        </figure>
      )}
    </header>
